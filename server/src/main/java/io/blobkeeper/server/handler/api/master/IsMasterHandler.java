...
@Singleton
public class IsMasterHandler implements RequestHandler<Boolean, EmptyRequest> {

    @Inject
    private ClusterMembershipService membershipService;

    @Override
    public ReturnValue<Boolean> handleRequest(@NotNull String data) {
        return new ReturnValue<>(membershipService.isMaster());
    }
}
