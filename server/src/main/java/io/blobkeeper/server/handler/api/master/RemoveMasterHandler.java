...
@Singleton
public class RemoveMasterHandler extends BaseRequestHandler<Boolean, EmptyRequest> {
    private static final Logger log = LoggerFactory.getLogger(RemoveMasterHandler.class);

    @Inject
    private ClusterMembershipService clusterMembershipService;

    @Override
    protected ReturnValue<Boolean> handlerRequest(@NotNull EmptyRequest request) {
        log.info("Remove master");

        if (!clusterMembershipService.tryRemoveMaster()) {
            return new ReturnValue<>(createError(SERVICE_ERROR, "Can't remove master"));
        } else {
    ...
    @Override
    protected Class<? extends EmptyRequest> getRequestClass() {
